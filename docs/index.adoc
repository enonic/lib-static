...
2. In order for index fallbacks to work properly:
  ...
  - URIs to the endpoint itself should return an empty string (unless there's a trailing slash, in which case only the slash should be returned),
  - URIs to other content below the endpoint should return a path beginning with a slash.
...
[[example-webapp]]
=== 6. A webapp with lib-router

Lib-static can also be used to serve assets on URIs directly below an link:https://developer.enonic.com/docs/xp/stable/runtime/engines/webapp-engine#webapp_js[XP webapp].

The easiest way is to use link:https://market.enonic.com/vendors/enonic/router-lib[the Router lib] to handle sub-URIs and extract a path relative to the webapp (`libRouterPath`), and let <<#example-getcleanpath, .getCleanPath>> fetch that from the request and return it - automatically handling the `.getCleanPath` <<#example-gotchas, gotchas>>.

This example serves a webapp from the URI `<webappRoot>`, and the frontend can fetch assets from `<webappRoot>/static/*`:
...
// Instead of exports.get, libRouter takes over:
libRouter.get(`/`, request => {
    const assetsRootUrl = `${request.contextPath}/static`;
    return {
        body: `
<html>
    <head>
        <title>Webapp</title>
        <link rel="stylesheet" type="text/css" href="${assetsRootUrl}/styles.css" />
    </head>

    <body>
        <h1>My webapp</h1>
        <img src="${assetsRootUrl}/images/my-logo.jpg" />
    </body>
</html>
        `
    };
...
[NOTE]
====
Another gotcha: in this example, `request.contextPath` is used as a simple but stable way to get the webapp's own root URL, aka `<webappRoot>`. Other approaches may be needed for other contexts (e.g. vhosting, etc), but either way:

Since `<webappRoot>/static` is directly below the webapp's root, *why resolve the webapp's root URL, instead of just using a relative path*? Why not just use `static/styles.css` and `static/images/my-logo.jpg`?

The XP webapp engine makes the controller respond identically to both `<webappRoot>` and `<webappRoot>/`. That trailing slash makes for two different contexts and resolves to two different relative addresses, so one stable, explicit value is needed.
====
