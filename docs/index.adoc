...
Bottom line: <<#example-getcleanpath, .getCleanPath>> can just fetch and return that from `request`, and the `.getCleanPath` <<#example-gotchas, gotchas>> are automatically handled (except for index fallback at the root - more on that <<#index-fallback-route, here>>).
...
==== Special case 1: webapp route with root-index-fallback

TIP: This example takes special care to handle trailing slashes at the end of the root URI - the difference between `/static` and `/static/`. Pay close attention to that.

The way lib-router works, `'/static/{libRouterPath:.+}'` will make it respond to sub-URIs after `static/` - that is, some sub-URI _and_ the slash is required. So in the example above, `<webappRoot>/static` and `<webappRoot>/static/` won't work (they will just return a 404).

But let's say we for some reason wanted that route to use lib-static's <<#index-fallback, index fallback functionality>> at the root of `/static`, not only handle the sub-URIs. More precisely, we want to expand the example above so that lib-static can make `<webappRoot>/static` redirect to `<webappRoot>/static/`, and `<webappRoot>/static/` respond with (the contents of) a file `static/index.html`.
...
==== Special case 2: avoid overlapping paths with files under /assets/

In the current versions of enonic XP, link:https://developer.enonic.com/docs/xp/stable/runtime/engines/asset-service#asset_files[the asset-serving engine serves files at a URL below the webapp], from the _src/main/resources/assets_ folder. This has prescedence before the _webapp.js_ controller.

That means that if you use the examples and patterns above to define your own route `libRouter.get('/$$_$$/asset/'), ...` and similar, and that route happens to match an existing file below the _/assets/_ folder, then the route will be ignored and you will get the file from the asset service instead. A narrow scenario, but could be confusing.

Simply avoid defining routes starting with `$$_$$/asset/` (or just don't start with an underscore).
